...
const totalAssetsParaCarregar = 2; //
...
imagemPersonagem.src = 't3za-character.png';
imagemPersonagem.onerror = () => {
    console.error("Erro ao carregar a imagem do personagem. Verifique o caminho: " + imagemPersonagem.src);
    desenharPersonagemFallback(canvasPersonagem.width / 2, canvasPersonagem.height / 2);
    verificarCarregamentoCompleto(); // Contar asset como "carregado" mesmo com erro
};

imagemPersonagem.onload = () => {
    console.log("Imagem do personagem carregada com sucesso.");
    desenharPersonagem(); // Desenha o personagem uma vez após o carregamento
    verificarCarregamentoCompleto(); // Contar asset como carregado
};
